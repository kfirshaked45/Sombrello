...
export function TaskSublist({ checklist, task, boardId, groupId }) {
  ...
  async function handleCheckboxChange(todoId) {
    ...
    if (checklistIndex !== -1) {
      const todoIndex = updatedTask.checklists[checklistIndex].todos.findIndex(
        (t) => t.id === todoId
      )
      if (todoIndex !== -1) {
        updatedTask.checklists[checklistIndex].todos[todoIndex].isDone =
          !updatedTask.checklists[checklistIndex].todos[todoIndex].isDone
        try {
          await updateTask(updatedTask, boardId, groupId)
        } catch (err) {
  ...
  async function handleTodoDelete(todoId) {
    ...
    if (checklistIndex !== -1) {
      updatedTask.checklists[checklistIndex].todos = updatedTask.checklists[
        checklistIndex
  ...
  async function handleDelete() {
    const updatedTask = { ...task }
    updatedTask.checklists = updatedTask.checklists.filter(
      (c) => c.id !== checklist.id
    )
    try {
      await updateTask(updatedTask, boardId, groupId)
  ...
  return (
    <section className="task-details-sublist">
      <div className="sublist-header">
        <h3>
          {<BsCheck2Square />} {checklist.title}
        </h3>
